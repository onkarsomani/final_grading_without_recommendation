...
function best(input) {
    let mini = Infinity;
    let ans = [];
    let per100 = Math.ceil(n / 100);
    for (let v of input) {
        let tot = 0;
        let f = 0;
        for (let i = 0; i < 7; i++) {
            if (i === 6) {
                if (ps[100] - ps[v[i] - 1] <= 5 * per100 || ps[100] - ps[v[i] - 1] >= 30 * per100) {
                    f = 1;
                    break;
                }
            } else if (i >= 4) {
                ...
                if (ps[v[i + 1] - 1] - ps[v[i] - 1] < 4 * per100 || ps[v[i + 1] - 1] - ps[v[i] - 1] >= 35 * per100) {
                    f = 1;
                    break;
                }
            } else if (i >= 1) {
                if (ps[v[i + 1] - 1] - ps[v[i] - 1] < 1 * per100 || ps[v[i + 1] - 1] - ps[v[i] - 1] >= 35 * per100) {
                    f = 1;
                    break;
                }
            } else {
            ...
            if (i <= 1) {
                tot += 0.1 * (ps[v[i] + 2] - ps[Math.max(v[i] - 2, dcut)]);
            } else if (i <= 4) {
                tot += 0.4 * (ps[v[i] + 2] - ps[Math.max(v[i] - 2, dcut)]);
            } else {
                tot += 0.2 * (ps[Math.min(v[i] + 2, 100)] - ps[Math.max(v[i] - 2, dcut)]);
            }
        }
        if (!f) {
            cccnt++;
